...
int main(void) {
    ...
    if (!arena_allocator) {
        printf("Error: Failed to initialize Arena allocator\n");
        return 1;
    }

    nu_mm_t *slob_allocator = nu_mm_create(NU_MM_SLOB, slob_backing, SLOB_POOL_SIZE);
    if (!slob_allocator) {
        printf("Error: Failed to initialize SLOB allocator\n");
        return 1;
    }
    ...
    if (scores && msg) {
        ...
        printf("Arena data: scores[4] = %d, msg = %s\n", scores[4], msg);
    }
    ...
    if (block_a && block_b) {
        printf("SLOB allocations successful at %p and %p\n", block_a, block_b);
    }
    ...
    if (block_c) {
        printf("SLOB successfully reused fragmented space at %p\n", block_c);
    }
    ...
    printf("Memory managers successfully torn down\n");
    return 0;
}
