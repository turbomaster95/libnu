        ...
        if (arg[0] == '-' && arg[1] != '\0') {
            ...
            if (def->type == NU_ARG_BOOL) {
                ...
                if (def->type == NU_ARG_STR) {
                    ...
                    if (!valid_int) {
                        nu_printf("Error: Value '%s' passed to flag option '%s' is not a valid integer\n", val_str, arg);
                        return false;
                    }
                }
            }
            ...
            if (ap->positional_count >= ap->positional_cap) {
                size_t old_size = sizeof(char*) * ap->positional_cap;
                ap->positional_cap *= 2;
                const char **new_pos = (const char**)nu_alloc(ap->mm, sizeof(char*) * ap->positional_cap);
